...
function transform(line1,line2,ctx,init,color1,color2){
	var Rprime = posToCart(init,line1);
	var R = posToCart(init,line2);
	var deltT = Rprime[0][1]-Rprime[0][0];
	var deltX = Rprime[1][1]-Rprime[1][0];
	var u = deltX/deltT;
	
	if(u<1 &&u>-1){
		var newLine = cartToPos(init,lorentzTransform(u,Rprime));
		var newLine2 = cartToPos(init,lorentzTransform(u,R));
		drawLine(newLine, ctx, color1,"R'");
    	ctx.fillStyle = color1;
    	ctx.fillRect(newLine[1][1]-4,newLine[0][1]-4,8,8);
    	var newLine = cartToPos(init,lorentzTransform(u,Rprime));
		drawLine(newLine2, ctx, color2,"R");
    	ctx.fillStyle = color2;
    	ctx.fillRect(newLine2[1][1]-4,newLine2[0][1]-4,8,8);
