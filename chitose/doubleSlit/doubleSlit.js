...
class Plot{
	...
	printNums(){
		...
		document.getElementById("slitsReading").innerHTML = Math.floor(this.setUp.slitApart*10**(9) )+ "micro meter";
...
var setUp = {//object that defines the current set up of double slit experiment
	...
	slitApart: 45*10**(-8), // distance between slits in cm
...
bwSlit.oninput=function(){
	plt.setUp.slitApart = parseFloat(bwSlit.value)*10**(-8);
	plt.plot();
	plt.drawInterference();
	plt.printNums();
